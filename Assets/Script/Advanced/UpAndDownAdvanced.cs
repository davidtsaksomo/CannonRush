...
using ChartboostSDK;
public class UpAndDownAdvanced : MonoBehaviour {
	...
	void OnCollisionEnter2D(Collision2D coll) {
		...
		if (coll.gameObject.name == "BolaMeriamAdvanced(Clone)") {
			bgm.GetComponent<AudioSource>().Stop();
			scoreanda = GetComponent<SpawnObstacleAdvanced> ().nilai;
			highscoreTable.AddScore(PlayerPrefs.GetString("PlayerName", "Jack"), scoreanda, "advanced");
			sounds[0].GetComponent<AudioSource>().Play();
			verSpeed = 0;
			horSpeed = 0;
			Destroy (GetComponent<SpawnObstacleAdvanced> ());
	...
	void death (){	
		...
		if (!PlayerPrefs.HasKey ("expert") && scoreanda> 49) {
			achievements[0].SetActive(true);
			achievements[5].GetComponent<Text>().text = "\nCongratulations!\n\nYou've Unlocked\n\nExpert Mode!";
			PlayerPrefs.SetInt("expert",1);
		}
		if (!PlayerPrefs.HasKey ("silver1") && scoreanda> 99) {
			achievements[0].SetActive(true);
			achievements[1].SetActive(true);
			PlayerPrefs.SetInt("silver1",1);

		}
		if (!PlayerPrefs.HasKey ("silver2") && scoreanda> 149) {
			achievements[0].SetActive(true);
			achievements[2].SetActive(true);
			PlayerPrefs.SetInt("silver2",1);
