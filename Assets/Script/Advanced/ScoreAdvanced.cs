...
public class ScoreAdvanced : MonoBehaviour {
	...
	void Start(){
		score = cube.GetComponent<UpAndDownAdvanced> ().scoreanda;
		highScore = PlayerPrefs.GetInt("highScoreYeahAdvanced",0);
	...
	void OnDisable(){
		...
		if(score>highScore){
			PlayerPrefs.SetInt("highScoreYeahAdvanced", score);
			PlayerPrefs.Save();
		}
	}
	...
	public IEnumerator ShareScreenshot()
	{
		...
		if(!Application.isEditor)
		{
			...
			intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>("EXTRA_TEXT"), ("Cannon Rush: My score is "+score + " on Advanced level! Come play and beat my score if you can! Download now from SlideMe Market! http://slideme.org/application/cannon-rush"));
			intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>
