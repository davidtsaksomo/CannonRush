...
public class Score : MonoBehaviour {
	...
	void Start(){
		score = cube.GetComponent<UpAndDown> ().scoreanda;
		highScore = PlayerPrefs.GetInt("highScoreYeah",0);
	...
	void OnDisable(){
		...
		if(score>highScore){
			PlayerPrefs.SetInt("highScoreYeah", score);
			PlayerPrefs.Save();
		}
	}
	...
	{print("bjbjbjbjb");
		...
		if(!Application.isEditor)
		{
			...
			intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>("EXTRA_TEXT"), ("Cannon Rush: My score is "+score + " on Classic level! Come play and beat my score if you can! Download now from SlideMe Market! http://slideme.org/application/cannon-rush"));
			intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>
